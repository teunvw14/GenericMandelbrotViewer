...
// TODO: take center_x and center_y variables into account
__global__ void build_complex_grid_kernel(double center_x, double center_y, double draw_radius, int resolution_x, int resolution_y, thrust::complex<double>* points, thrust::complex<double>* iterated_points) {
...
__global__ void mandelbrot_iterate_and_color_kernel(
    int max_iterations, 
...
) 
{
    ...
    for (int pixel_y = block_index_x; pixel_y < resolution_y; pixel_y += block_stride_x)
    {
        for (int pixel_x = thread_index_x; pixel_x < resolution_x; pixel_x += thread_stride_x)
        {
            ...
            index = pixel_y * resolution_y + pixel_x;
            // TODO: Optimize
            while (iterations < max_iterations && squared_absolute_values[index] < escape_radius_squared) {
                iterated_points[index] = iterated_points[index] * iterated_points[index] + points[index]; // z^2 + c
                squared_absolute_values[index] = iterated_points[index].real() * iterated_points[index].real() + iterated_points[index].imag() * iterated_points[index].imag();
                iterations++;
            }
            ...
            {
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 0] = 0; // Red value
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 1] = 0; // Green value
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 2] = 0; // Blue value
            }
            else
            {
                unsigned char red = iterations*iterations + 255 / squared_absolute_values[index];
                // cap to 255
                if (red > 255)
                    red = 255;
                else if (red < 0)
                ...
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 0] = red; // Red value
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 1] = green; // Green value
                rgb_data[3 * (resolution_y * pixel_y + pixel_x) + 2] = blue; // Blue value
            }
                    }
    }
}
...
double escape_radius_squared = 4; // escape_radius = 2
